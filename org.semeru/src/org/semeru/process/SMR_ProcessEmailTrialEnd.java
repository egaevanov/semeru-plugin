...
public class SMR_ProcessEmailTrialEnd extends SvrProcess{
			...
			try {
				...
				while (rs.next()) {
					...
					if(p_Next_DueDate.compareTo(p_Now) < 3){
						...
						sendIndividualMail(user.getName(), AD_User_ID, rs.getTimestamp(2),pkg);
	 ...
	 */
	private Boolean sendIndividualMail (String Name, int AD_User_ID, Timestamp dueDate,X_mob_mst_package Package)
	{
		...
		message.append("\n");
		message.append("\n");
		message.append("Team Semeru Ganteng");
